...
function show_day( $timestamp ){
  return strftime('%A, den %d. %B %Y', $timestamp);
}

function show_time( $timestamp ){
  $return = strftime('%H:%M Uhr', $timestamp);

  if( $return != '00:00 Uhr' AND $return != '24:00 Uhr') 
    return $return;
}
...
/*
    if(date('G',$meta_angabe_zeit['_zeitstempel'][0])==0){
      ...
      $echo .= "</span>\n";
          ...
          $echo .= strftime('<span class="termin_tag">%A, den %e. %B</span><span class="termin_zeit">%I:%M Uhr</span>', $meta_angabe_zeit['_zeitstempel'][0]);
           $echo .= "</span>\n";
      }
  */    
     
      $echo .= '<span class="termin_tag">'. show_day($meta_angabe_zeit['_zeitstempel'][0]) . '</span>';
      $echo .= ' <span class="termin_zeit">,'. show_time($meta_angabe_zeit['_zeitstempel'][0]) . '</span>';

      if( $meta_angabe_zeit['_bis'][0] != ''){
        $echo .= ' bis <span class="termin_tag">'. show_day(strftime( strToTime( $meta_angabe_zeit['_bis'][0]) ) )  . '</span>';
        $echo .= ' <span class="termin_zeit">'. show_time(strftime( strToTime( $meta_angabe_zeit['_bis'][0]) ) )  . '</span>';
      }
/*
     $echo .= utf8_encode( strftime('

      <span class="termin_tag">%A, den %d. %B %Y </span>
      <span class="termin_zeit">%H:%M Uhr',$meta_angabe_zeit['_zeitstempel'][0]).
     ' bis '.$meta_angabe_zeit['_bis'][0] );
...
function the_termin_widget( $clock, $city){
  setlocale(LC_ALL, 'de_DE.UTF-8');
  $meta_angabe_zeit=get_post_custom( get_the_ID());
  ...
  echo "\n\n<div class='termin_meta_kurz'>\n";
      ...
      echo '<span class="termin_tag">'. show_day($meta_angabe_zeit['_zeitstempel'][0]) . '</span>';
       if( $clock ) echo ' <span class="termin_zeit">,'. show_time($meta_angabe_zeit['_zeitstempel'][0]) . '</span>';
     if($city AND isset($meta_angabe_zeit['_geostadt']) AND $meta_angabe_zeit['_geostadt'][0]!=''){ echo '<br/><span class="termin_ort_kurz">'.$meta_angabe_zeit['_geostadt'][0].'</span>'; }
...
function wpcal_add_content( $content ) {
    ...
    $meta_angabe=get_post_custom( get_the_ID());

    $custom_content = the_termin();
    $custom_content .= $content;
    if($meta_angabe['_showmap'][0] == "on")    $custom_content .= the_termin_geo(); 
